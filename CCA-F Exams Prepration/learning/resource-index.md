# CCA-F Resource Index

## Required Starting Point

- Guide PDF: [guide_en.pdf](../guide/guide_en.pdf)

## Concept Tutorials

- YouTube tutorial list with 20 modules: [Youtube Link.txt](../Youtube%20Tutorial/Youtube%20Link.txt)

## Slides

- Local slide decks: [slide decks](../slide%20decks/)

## Official Documentation

- Claude Code overview: <https://code.claude.com/docs/en/overview>
- Anthropic learning portal: <https://anthropic.skilljar.com/>

## Practice Tests

- Free practice questions: <https://claudecertifications.com/claude-certified-architect/practice-questions#main-content>
- Paid Udemy practice tests:
  - <https://www.udemy.com/course/new-claude-certified-architect-foundations-cca-f-exams/?couponCode=KEEPLEARNING>
  - <https://www.udemy.com/course/claude-certified-architect-practice-tests/>
  - <https://www.udemy.com/course/anthropic-claude-certified-architect-3-full-practice-exams/>

## Notes for Maintainers

- Keep large binary decks in `CCA-F Exams Prepration/slide decks/`.
- Keep learner-facing navigation and checklist files in this `learning` folder.
- Avoid adding exam-protected material. Store only legitimate preparation resources and original notes.
